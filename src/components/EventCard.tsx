...
const EventCard = ({ event, index = 0 }: EventCardProps) => {
  ...
  return (
    <AnimatedElement animation="slide-up" delay={delay} threshold={0.1}>
      <div className={`rustic-card overflow-hidden card-animate transition-opacity ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
        {imagePath && (
          <div 
        ...
        <div className="p-4 md:p-6">
          ...
          <div className="flex space-x-2">
            <Button className="w-1/2 bg-sage hover:bg-sage/90 text-white text-sm py-2 h-auto btn-hover-effect">
              Add to Calendar
            </Button>
            <Link to={`/events/${id}`} className="w-1/2">
              <Button variant="outline" className="w-full border-sage text-sage hover:bg-sage/10 text-sm py-2 h-auto btn-hover-effect">
                View Details
              </Button>
            </Link>
          </div>
